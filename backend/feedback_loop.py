...
class QueryFeedbackLoop:
    ...
    def process_query_with_feedback(self, question: str, initial_sql: str = None) -> Dict[str, Any]:
        """
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if validation.is_valid:
                ...
                success = True
                logger.info(f"Query validated successfully on attempt {attempt_number + 1}")
                break
            
            if attempt < self.max_retries:
                # Generate improved SQL based on validation feedback
                improved_sql = self._generate_improved_sql(question, current_sql, validation)
                if improved_sql and improved_sql != current_sql:
                    current_sql = improved_sql
                    logger.info(f"Generated improved SQL for attempt {attempt_number + 2}")
                else:
                    logger.warning(f"Failed to generate improved SQL for attempt {attempt_number + 2}")
                    break
            else:
                logger.warning(f"Max retries reached ({self.max_retries}) without success")
    ...
    def _generate_improved_sql(self, question: str, failed_sql: str, 
                              validation: ValidationFeedback) -> Optional[str]:
        """Generate improved SQL based on validation feedback"""
        try:
...
"""
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for query in self.retry_history:
            for result in query['validation_results']:
                if not result['validation']['is_valid']:
                    ...
                    validation_failures[result_type] = validation_failures.get(result_type, 0) + 1
        ...
        return {
            ...
            'retry_efficiency': round(successful_queries / total_attempts * 100, 2) if total_attempts > 0 else 0
        }
