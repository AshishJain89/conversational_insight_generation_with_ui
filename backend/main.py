...
from .insight_generator import create_insight_generator, generate_insights_safe

# forecasting imports
import pandas as pd, numpy as np, io, base64, traceback, matplotlib, matplotlib.pyplot as plt
from concurrent.futures import ThreadPoolExecutor, TimeoutError as FuturesTimeoutError

from backend import insight_generator
...
async def lifespan(app: FastAPI):
    global chain, schema, validator, feedback_loop, insight_generater
    try:
        ...
        logger.info("Initializing chart suggester and insight generator")
        chart_suggester = ChartSuggester()
        insight_generator = create_insight_generator(timeout_seconds=30, enable_cache=True)
        logger.info("Chart suggester and insight generator initialized successfully!")
...
def get_chain():
    if chain is None:
        raise HTTPException(
            status_code=503, detail="Service not ready - LLM chain not initialized"
        )
    return chain
...
def get_schema():
    if schema is None:
        raise HTTPException(
            status_code=503, detail="Service not ready - Database schema not loaded"
        )
    return schema
...
def get_validator():
    if validator is None:
        raise HTTPException(
            status_code=503,
            detail="Service not ready - Query validator not initialized",
        )
    return validator
...
def get_feedback_loop():
    if feedback_loop is None:
        raise HTTPException(
            status_code=503, detail="Service not ready - Feedback loop not initialized"
        )
...
def get_chart_suggester():
    if chart_suggester is None:
        raise HTTPException(
            status_code=503,
            detail="Service not ready - Chart suggester not initialized",
        )
...
def process_with_feedback(
    query: GenerateSQLQuery, db_feedback_loop=Depends(get_feedback_loop)
):
    ...
    try:
        ...
        if result["success"] and query.execute and result["final_sql"]:
            try:
                ...
                logger.info(
                    f"Feedback loop SQL executed successfully, returned {len(rows)} rows"
                )
            except Exception as e:
        ...
        if query.execute and sql:
            ...
            if not validation_feedback.is_valid:
                ...
                if validation_feedback.suggestions:
                    result.message += (
                        f". Suggestions: {', '.join(validation_feedback.suggestions)}"
                    )
                result.type = "text"
            ...
            try:
                ...
                # Generate insights from the data (regardless of forecast intent)
                try:
                    insights = generate_insights_safe(
                        query.question, columns, rows, insight_generator
                    )
                ...
                if forecast:
                    ...
                    if has_date_column:
                        result.type = (
                            "forecast"  # Use forecast type for historical time-series
                        )
...
class ForecastRequest(BaseModel):
    ...
    rows: Optional[List[List[Any]]] = Field(
        default=None, description="Row-major data matching columns"
    )
    horizon: int = 12
    ...
    missing_method: Optional[str] = Field(
        "interpolate", description="interpolate|ffill|bfill|drop"
    )
...
def _detect_columns(
    ...
    if not date_col or not value_col:
        raise ValueError(
            "Could not autodetect date_col and/or value_col. Please provide them."
        )
...
def _forecast_and_plot(model, ts: pd.Series, horizon: int, freq: str):
    ...
    if np.any(mean < reasonable_range[0]) or np.any(mean > reasonable_range[1]):
        raise ValueError(
            "Forecast values are outside reasonable bounds - model may be overfitting"
        )
...
def api_forecast(req: ForecastRequest):
    if req.horizon > MAX_HORIZON:
        raise HTTPException(
            status_code=400, detail=f"horizon too large, max {MAX_HORIZON}"
        )
        ...
        try:
            ...
            if req.sql:
                ...
                if req.columns is None or req.rows is None:
                    raise ValueError(
                        "Either sql must be provided, or columns and rows must be supplied"
                    )
            ...
            df = pd.DataFrame(rows, columns=columns)
            date_col, value_col = _detect_columns(
                columns, df, req.date_col, req.value_col
            )
            ts, freq = _prepare_series(
                columns, rows, date_col, value_col, req.freq, req.missing_method
            )
            ...
            diagnostics = {
                ...
                "residuals_mean": (
                    float(np.mean(model.resid)) if hasattr(model, "resid") else None
                ),
                "rmse": (
                    float(np.sqrt(np.mean(np.square(model.resid))))
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app", host=HOST, port=PORT, log_level=LOG_LEVEL.lower(), reload=True
    )
