...
class ARIMAModelSelector:
    ...
    def select_and_fit(
        self, ts: pd.Series, seasonal: bool = None, m: int = None
    ) -> Tuple:
        """
    ...
    def _detect_seasonality(
        self, ts: pd.Series, seasonal: bool, m: int
    ) -> Tuple[bool, int]:
    ...
    def _try_auto_arima(self, ts: pd.Series, seasonal: bool, m: int) -> Tuple:
        ...
        arima = self.auto_arima(
            ...
            start_Q=1,
            d=d,
            D=D,  # Force specific differencing
            trace=False,
        )
    ...
    def _manual_model_selection(self, ts: pd.Series, seasonal: bool, m: int) -> Tuple:
        ...
        if seasonal and m == 12:
            ...
            for p, d, q, P, D, Q, s in seasonal_models:
                try:
                    model = self.ARIMA(
                        ts, order=(p, d, q), seasonal_order=(P, D, Q, s)
                    ).fit()
...
def test_model_selector():
    ...
    values = (
        3000000
        + np.random.normal(0, 200000, len(dates))
        + 500000 * np.sin(np.arange(len(dates)) * 2 * np.pi / 12)
    )  # Add seasonality
    ...
    try:
        ...
        for i, (date, value) in enumerate(
            zip(forecast.predicted_mean.index, forecast.predicted_mean.values)
        ):
