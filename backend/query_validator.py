...
class QueryValidator:
    ...
    def _load_schema(self):
        ...
        try:
            ...
            conn.close()
            logger.info(
                f"Schema loaded: {len(tables)} tables, {sum(len(cols) for cols in schema.values())} columns"
            )
        ...
        for table in tables:
            if table not in self.schema_cache:
                return ValidationFeedback(
                    ...
                    suggestions=[
                        f"Available tables: {', '.join(self.schema_cache.keys())}"
                    ],
                )
            ...
            if actual_table and actual_table in self.schema_cache:
                if column not in self.schema_cache[actual_table]:
                    return ValidationFeedback(
                        ...
                        suggestions=[
                            f"Available columns in {actual_table}: {', '.join(self.schema_cache[actual_table])}"
                        ],
                    )

        return ValidationFeedback(is_valid=True, result=ValidationResult.VALID)

    def _find_table_by_alias(
        self, sql: str, alias: str, tables: List[str]
    ) -> Optional[str]:
        ...
        try:
            ...
            if not explain_result:
                return ValidationFeedback(
                    ...
                    suggestions=[
                        "Check SQL syntax and ensure all referenced objects exist"
                    ],
                )
    ...
    def _analyze_performance(self, sql: str) -> ValidationFeedback:
        ...
        if join_count > 3:
            complexity_score += 2
            suggestions.append(
                "Consider breaking down complex multi-table joins into smaller queries"
            )
        ...
        if "(" in sql and "SELECT" in sql:
            complexity_score += 1
            suggestions.append(
                "Consider using CTEs (WITH clause) instead of subqueries for better readability"
            )
        ...
        if "GROUP BY" in sql_upper:
            complexity_score += 1
            suggestions.append(
                "Ensure GROUP BY columns are properly indexed for better performance"
            )

        # Check for LIKE with wildcard at start
        if "LIKE" in sql_upper and re.search(r"LIKE\s+['\"]%", sql):
            suggestions.append(
                "Avoid leading wildcards in LIKE clauses as they prevent index usage"
            )
    ...
    def _get_error_suggestions(self, error_msg: str) -> List[str]:
        ...
        if "no such table" in error_lower:
            ...
            suggestions.append(
                "Review SQL syntax, check for missing keywords or incorrect punctuation"
            )
